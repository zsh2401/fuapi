...
export type ExpressAPIHandler<T extends API<any, any>> =
  T extends FullAPI<infer DTO, infer VO>
    ? (
        args: ExpressHandlerArgs<T['dtoSchema']>
      ) => Promise<T['voSchema']>
    : T extends NoRetAPI<infer VO>
      ? (
          dto: ExpressHandlerArgs<T['dtoSchema']>
        ) => Promise<void>
      : T extends NoArgAPI<infer VO>
        ? () => T['voSchema']
        : () => Promise<void>
 ...
 */
export function implAPIOnExpress<
  DTO extends ZodBase,
  VO extends ZodBase,
>(
  express: Express,
  api: FullAPI<DTO, VO>,
  handler: ExpressAPIHandler<
    FullAPI<z.output<DTO>, z.output<VO>>
  >
): void
 ...
 */
export function implAPIOnExpress<VO extends ZodBase>(
  express: Express,
  api: NoArgAPI<VO>,
  handler: ExpressAPIHandler<NoArgAPI<z.output<VO>>>
): void
/**
 * Implement your no returning api on express instance
 */
export function implAPIOnExpress<DTO extends ZodBase>(
  express: Express,
  api: NoRetAPI<DTO>,
  handler: ExpressAPIHandler<NoRetAPI<z.output<DTO>>>
): void

/**
 * Implement your no argument and returning api on express instance
 */
export function implAPIOnExpress<A extends NoIOApi>(
  express: Express,
  api: NotAllowed<A, NoArgAPI<any>>,
  handler: (dto: ExpressHandlerArgs<void>) => Promise<void>
): void
